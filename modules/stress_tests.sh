...
# GPU stress using Swift Metal Compute Shader (primary method, minimal CPU dependency)
start_gpu_stress() {
    ...
    if [ -f "$SWIFT_FILE" ]; then
        ...
        if [ $? -eq 0 ] && [ -f "/tmp/gpu_stress_bin" ]; then
            ...
            log "${RED}[GPU] 编译 Swift GPU 测试失败，跳过 GPU 测试${NC}"
            return 1
        fi
    else
...
stop_gpu_stress() {
    ...
    pkill -9 -f "/tmp/gpu_stress_bin" 2>/dev/null

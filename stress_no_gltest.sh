...
if [ -f "$DISK_LOG" ] && [ $(wc -l < "$DISK_LOG") -gt 1 ]; then
    ...
    log "平均吞吐量：${avg_mb:-0} MB/s"
    log "峰值吞吐量：${max_mb:-0} MB/s"
    log "平均 IOPS：${avg_tps:-0}"
    log "峰值 IOPS：${max_tps:-0}"
    
    is_low=$(awk -v avg="${avg_mb:-1000}" 'BEGIN { if(avg > 0 && avg < 50) print 1; else print 0 }')
    is_med=$(awk -v avg="${avg_mb:-1000}" 'BEGIN { if(avg >= 50 && avg < 200) print 1; else print 0 }')
